...
class BattleScreen extends Component {
	...
	componentDidMount() {
		...
		this.timer=setInterval(
			function() {
				console.log(this.state.redStat.attack,this.state.cur_red_hp,"what is redhp?");
				this.setState({
	...
	render() {
		let { redStat, blueStat, displayWinner } = this.state;
		 let redHealthPercent=(this.state.cur_red_hp/this.state.redStat.hp)*100;
		 let blueHealthPercent=(this.state.cur_blue_hp/this.state.blueStat.hp)*100;
		return (
			<div className="battleStage">
				<h3 style={{ color: '#111111' }}>{this.state.redStat.red_name}</h3>
				<ProgressBar active now={redHealthPercent<0?0:redHealthPercent} striped bsStyle="danger" />
				<h4 style={{ color: '#111111' }}>VS</h4>
				<h3 style={{ color: '#111111' }}>{this.state.blueStat.blue_name}</h3>
				<ProgressBar active now={blueHealthPercent<0?0:blueHealthPercent} striped bsStyle="info" />

{/*				 <BattleTime start={Date.now()} /> 
*/}				 <RedSide redP={redStat} />
				<BlueSide blueP={blueStat} /> 
				{this.state.displayWinner===true ?
					<div><div style={{ color: '#111111' }}>
